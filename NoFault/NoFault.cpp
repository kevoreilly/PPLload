...
// Returns whether the given process should receive additional protection
...
)
{
	...
	if (!IsCurrentProcessFullPPOrWinTcbLight() && ShouldHardenProcess(hProcess))
	{
...
)
{
	...
	CleanUpWithStatusIfFailed(PsSetCreateProcessNotifyRoutineEx(CreateProcessNotifyRoutine, FALSE));

	CleanUpWithStatusIfFailed(RegisterFilter(DriverObject));
...
Cleanup:
	// TODO: Upon failure, clean up notify routine if it was registered
