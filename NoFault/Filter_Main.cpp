...
RegisterFilter(_In_ PDRIVER_OBJECT pDriverObject)
{
    ...
    if (!NT_SUCCESS(ntStatus))
    {
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "NoFault: FltRegisterFilter() failed with status %08x", 
            ntStatus);
        goto Cleanup;
    }
    ...
    {
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "NoFault: FltStartFiltering() failed with status %08x",
            ntStatus);
    ...
    DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "NoFault: RegisterFilter() success\n");
...
)
{
    ...
    pBuffer = (PWCHAR)ExAllocatePool2(PagedPool, volumeNameLength, POOL_TAG);
    if (!pBuffer)
    {
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, 
        "NoFault: InstanceSetupCallback for Flags: 0x%x DevType: %u FS: %u for %wZ\n",
